...
string containtment(Point p,Rectangle r)
{
    ...
    if(p.x > r.xmin and p.x <r.xmax and p.y < r.ymax and p.y > r.ymin)return "inside";
    if(p.x < r.xmin or p.x >r.xmax or p.y > r.ymax or p.y < r.ymin) return "outside";
    return "border";
...
int main()
{
    ...
    for(int i = 0; i<n; ++i){
        ...
        Point p;
        Rectangle r;
        cin >> p.x >> p.y >> r.xmin >> r.ymin >> r.xmax >> r.ymax;
        string s = containtment(p,r);
        cout << s << endl;
